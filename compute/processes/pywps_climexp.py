...
class KnmiClimateExplorerWpsProcess(WPSProcess):
    ...
    def execute(self):
        ...
        self.status.set("Setting provenance", 20)
        ...
        self.status.set("Correlating", 30)
        climexp_pyapi.correlatefield(self.inputs)

        self.status.set("Setting output", 60)
    ...
    #except Exception, e:
        ...
        #
        # 1) Move out.nc from current temporary working directory to basket directory. 
        # 2) Construct an opendap URL pointing to this data file.
        #
        pathToBasket = os.environ['POF_OUTPUT_PATH']
        if pathToBasket is not None:
          uniqueFolderName=self.pywps.UUID          
          basket = tempfile.mkdtemp(prefix=uniqueFolderName, dir=pathToBasket)
          urlToBasket  = os.environ['POF_OUTPUT_URL']+"/"+basket[len(pathToBasket):]
          os.rename("out.nc", basket + "/out.nc")
          self.outputs['data'].setValue(urlToBasket+"/out.nc") # This is an opendap accessible URL to the data file in the basket.
        else:
          self.outputs['data'].setValue("out.nc") # Fallback default.

        self.status.set("Setting output", 80)
